...
def kind_libfmt_format(kind):
    if kind == '8BitRegister':
        return '{}'
    elif kind == '16BitRegister':
        return '{}'
...
class Opcode:
    ...
    def getfunctioncall(self):
        ...
        args = [arg.getcppsymbol() for arg in self.args.args
                if not kind_isimmediate(arg.kind)
                and not arg.kind == 'JumpCondition']
    ...
    def toStr(self):
        return "instruction: {}; value: {}; cycles: {}; format: {}, nargs: {}, size: {}; args: {};" \
            .format(self.instruction, self.value, self.cycles, self.fmt, self.nargs, self.size, self.args.toStr())
